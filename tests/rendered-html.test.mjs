...
test("renders the complete Humberto Zizi portfolio", async () => {
  ...
  assert.match(page, /title: "menorDesconto"/);
  assert.doesNotMatch(page, /title: "(?:CSGORoll Alerts|WorkSite)"/);
...
test("opens the Foto Imas project with only its visual README", async () => {
  ...
  assert.match(page, /function DiscountReadmePreview/);
  assert.doesNotMatch(page, /storeSourceFiles|filesMenuOpen|readmeMode|store-code-file-picker/);
  assert.match(page, /isReadmeCase \? \(\s*<StoreCodeWorkspace/);
  assert.match(page, /isReadmeCase \? "is-store-theme"/);
  assert.match(page, /!isReadmeCase && <nav className="studio-floating-steps"/);
  ...
  assert.match(page, /store-readme-result/);
  assert.match(page, /activeReadmeSection/);
  assert.match(page, /syncVisibleProject/);
  ...
  assert.doesNotMatch(page, /aaPanel/i);
  ...
  const heroActionBlocks = [...page.matchAll(/<div className="store-readme-actions">([\s\S]*?)<\/div><\/div>/g)].map((match) => match[1]);
  assert.equal(heroActionBlocks.length, 2);
  for (const actions of heroActionBlocks) {
    assert.equal((actions.match(/<a /g) ?? []).length, 1);
    assert.match(actions, /MonitorPlay/);
    assert.doesNotMatch(actions, /LayoutDashboard|ver dashboard/);
  }
  const floatingActions = page.match(/<nav className="store-floating-actions"[\s\S]*?>([\s\S]*?)<\/nav>/)?.[1] ?? "";
  assert.equal((floatingActions.match(/<a /g) ?? []).length, 1);
  assert.match(floatingActions, /MonitorPlay/);
  assert.doesNotMatch(floatingActions, /LayoutDashboard|ver dashboard/);
  const dashboardCaptions = [...page.matchAll(/<figcaption>([\s\S]*?)<\/figcaption>/g)].map((match) => match[1]);
  assert.equal(dashboardCaptions.length, 2);
  for (const caption of dashboardCaptions) assert.match(caption, /LayoutDashboard[\s\S]*?ver dashboard/);
  ...
  assert.match(page, /function HistoryTypingConsole/);
  assert.match(page, /commit_history\.console/);
  assert.match(page, /setTypedText\(text\.slice\(0, characterIndex\)\)/);
  assert.match(page, /<HistoryTypingConsole text=\{t\.historyDescription\} \/>/);
  assert.match(styles, /\.history-console\s*\{/);
  assert.match(styles, /@keyframes history-console-cursor/);
  ...
  assert.match(page, /ShoppingBag/);
  assert.match(page, /PackageSearch/);
  assert.match(page, /LayoutDashboard/);
  assert.match(page, /CircleCheckBig/);
  assert.match(page, /FaGithub/);
  assert.match(page, /humberto\.fae@icloud\.com/);
  assert.match(page, /submitContactForm/);
  assert.match(page, /formsubmit\.co\/ajax/);
  assert.match(page, /contact-form-layer/);
  assert.match(page, /const goToProjects/);
  assert.match(page, /className="command-link" type="button" onClick=\{goToProjects\}/);
  assert.match(page, /className="primary-action" type="button" onClick=\{goToProjects\}/);
  assert.doesNotMatch(page, /contactMailto|contact-destination/);
  assert.match(page, /project-kicker/);
  assert.match(page, /CodeXml/);
  assert.match(page, /ChevronDown/);
  assert.doesNotMatch(page, /StoreStageConsole|storeStageRuns|store-stage-console/);
  ...
  assert.match(styles, /\.store-floating-actions/);
  assert.match(styles, /\.store-readme-delivery\s*\{[^}]*border-bottom:0/);
  assert.match(styles, /@keyframes store-floating-actions-in/);
  ...
  assert.match(styles, /top:calc\(50% - 78px\)[^}]*height:156px/);
  ...
  assert.doesNotMatch(styles, /\.store-stage-(console|code|execute|ready)/);
