...
function StoreReadmePreview({ language, visibleSections }: { language: Language; visibleSections: string[] }) {
  ...
  return <article className="store-readme-preview">
    <header id="store-readme-intro" ref={heroRef} className={reveal("store-readme-intro", "store-readme-hero")}>
      <div><span className="store-readme-kicker">README.md · PRODUCT CASE</span><h1>imas<em>Store</em></h1><p className="store-readme-hero-copy">{pt ? "Uma experiência completa para transformar fotografias em ímãs personalizados — da primeira escolha ao acompanhamento do pedido." : "A complete experience for turning photos into custom magnets — from the first choice to order tracking."}</p><div className="store-readme-actions"><a href={`${publicBasePath}/demos/foto-imas-store/index.html`} target="_blank" rel="noreferrer">{pt ? "ver demonstração" : "view demo"} ↗</a><a href={`${publicBasePath}/demos/foto-imas-store/index.html#/dashboard`} target="_blank" rel="noreferrer">{pt ? "ver dashboard" : "view dashboard"} ↗</a><a href="https://fotoimasstore.com.br/" target="_blank" rel="noreferrer">{pt ? "projeto publicado" : "live project"} ↗</a><a href="https://github.com/HumbertoFae/foto-imas-store" target="_blank" rel="noreferrer">{pt ? "ver código" : "view source"} ↗</a></div></div>
    ...
    <section id="store-readme-result" className={reveal("store-readme-result", "store-readme-section store-readme-delivery")}>
      ...
      <footer className="store-readme-footer"><span><CircleCheckBig aria-hidden="true" /></span><div><small>{pt ? "RESULTADO" : "RESULT"}</small><strong>{pt ? "Uma loja completa, responsiva e preparada para publicação, conectando a experiência do cliente à operação." : "A complete responsive storefront ready for publishing, connecting customer experience to operations."}</strong></div><a href="https://github.com/HumbertoFae/foto-imas-store" target="_blank" rel="noreferrer"><FaGithub aria-hidden="true" />{pt ? "ver no GitHub" : "view on GitHub"}</a></footer>
    </section>
  </article>;
}
